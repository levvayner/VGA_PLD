...
class DuePort
{
    ...
    private static string FileName = String.Empty;
        public DuePort(string fileName)
        {
    ...
    public void WriteBinary(bool pad = false)
    {
        try
        {
            ...
            Console.WriteLine("Passing program type as 1 ");
            buffer[0] = 1;
            ...
            for (int frameNum = 0; frameNum < numberOfFrames; frameNum++)
            {
                ...
                int acutalSize = data.Count - frameNum * frameSize >= frameSize ? frameSize : data.Count - frameNum * frameSize ;
                ...
                var result = ReadBytes(1, outStream);                    
            ...
            DateTime lastWriten = DateTime.Now;

            while(true){
                var messages = ReadBytes(3, outStream);
                foreach(var message in messages){
                    Console.WriteLine(message.ToString());
                    lastWriten = DateTime.Now;
                }
                if(DateTime.Now.Subtract(lastWriten).TotalSeconds > 30){
                    break;
                }
            }
    ...
    void EstablishProgrammingMode(Stream outStream)
    {
        ...
        //var outStream = port.BaseStream;
        ReadBytes(50, outStream,5); //get first ten into lines   
